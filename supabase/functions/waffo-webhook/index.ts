...
function buildInboundDiagnostic(req: Request, rawBody: string): InboundDiagnostic {
  return {
    received_at: new Date().toISOString(),
    method: req.method,
    headers_present: Array.from(req.headers.keys()).sort(),
    signature_header_present: Boolean(req.headers.get('x-waffo-signature')?.trim()),
    body_size: new TextEncoder().encode(rawBody).byteLength,
...
function warnInvalidWebhook(params: {
  ...
  eventType: string
  eventId: string
  inbound: InboundDiagnostic
}) {
  console.warn('waffo_webhook_rejected', params)
}

function isWebhookVerificationEvent(payload: Record<string, unknown>) {
  return getWebhookProductName(payload) === '[TEST] Webhook Verification'
    && getWebhookBuyerEmail(payload) === 'test-webhook@waffo.com'
}
...
async function findCheckoutSessionFallback(
  admin: ReturnType<typeof createAdminClient>,
  params: {
    mode: 'test' | 'prod'
    userId: string
  ...
  const { data: sessions } = await admin
    ...
    .select('id,local_order_id,user_id,product_id,plan_id,amount,currency,status,created_at')
    .eq('user_id', params.userId)
    .eq('mode', params.mode)
    .order('created_at', { ascending: false })
    .limit(8)

  const normalizedAmount = normalizeAmountValue(params.amount)
  const normalizedCurrency = normalizeCurrencyValue(params.currency)
  const usableStatuses = new Set(['created', 'opened', 'completed'])

  const candidates = (sessions ?? [])
    .filter((session) => usableStatuses.has(String(session.status ?? '')))
    .filter((session) => !params.productId || session.product_id === params.productId)
    .filter((session) => !params.planId || session.plan_id === params.planId)
    .filter((session) => {
      if (!normalizedAmount) return true
      return normalizeAmountValue(session.amount) === normalizedAmount
    })
    .filter((session) => {
      if (!normalizedCurrency) return true
      return normalizeCurrencyValue(session.currency) === normalizedCurrency
    })

  if (params.productId || params.planId) {
...
Deno.serve(async (req) => {
  ...
  const inbound = buildInboundDiagnostic(req, rawBody)
  console.info('waffo_webhook_inbound', inbound)

  let parsed: Record<string, unknown>
  try {
    parsed = JSON.parse(rawBody) as Record<string, unknown>
  } catch {
    console.warn('waffo_webhook_invalid_json', inbound)
  ...
  console.info('waffo_webhook_verified', {
    ...inbound,
    event_type: verifiedEvent.eventType,
    event_id: verifiedEvent.eventId,
...
async function processEvent(
  ...
  if (isWebhookVerificationEvent(payload)) {
    console.info('waffo_webhook_verification_event', {
      mode,
      eventType,
      orderId: getWebhookOrderId(payload),
    })
    return
  }
...
async function resolveBillingContext(
  ...
  const buyerIdentityUserId = getWebhookBuyerIdentityString(payload)
  ...
  const waffoProductId = getWebhookProductId(payload)
  const payloadAmount = getWebhookAmount(payload)
  const payloadCurrency = getWebhookCurrency(payload)
  const orderId = getWebhookOrderId(payload)
  ...
  if ((!resolvedPlanId || !resolvedProductId) && waffoProductId) {
    const { data: plans } = await admin
      .from('plans')
      .select('id,product_id')
      .eq('status', 'active')
      .or(`waffo_product_id_test.eq.${waffoProductId},waffo_product_id_prod.eq.${waffoProductId}`)
      .limit(2)
  ...
  if (!checkoutSession && resolvedUserId) {
    const fallbackSession = await findCheckoutSessionFallback(admin, {
      mode,
    ...
    if (fallbackSession) {
      checkoutSession = fallbackSession
      resolvedPlanId = typeof fallbackSession.plan_id === 'string' ? fallbackSession.plan_id : resolvedPlanId
      resolvedProductId = typeof fallbackSession.product_id === 'string' ? fallbackSession.product_id : resolvedProductId
    }
  }
  ...
  if (!resolvedUserId || !resolvedProductId || !resolvedPlanId) {
    ...
    const missingParts = [
      ...
      !resolvedProductId ? 'product_id' : null,
      !resolvedPlanId ? 'plan_id' : null,
    ].filter(Boolean).join(',')

    const metadataKeys = Object.keys(metadata)
    throw new Error(
      `Unable to resolve billing context for ${eventType}; missing=${missingParts || 'none'}; metadataKeys=${
        metadataKeys.length > 0 ? metadataKeys.join('|') : 'none'
      }; merchantProvidedBuyerIdentity=${merchantProvidedBuyerIdentity ? 'present' : 'missing'}; buyerIdentity=${
        buyerIdentityUserId ? 'present' : 'missing'
      }; waffoProductId=${waffoProductId ?? 'null'}; waffoPlanId=${waffoPlanId ?? 'null'}; waffoPriceId=${waffoPriceId ?? 'null'}; orderId=${
        orderId ?? 'null'
      }; productName=${getWebhookProductName(payload) ?? 'null'}`,
    )
  }

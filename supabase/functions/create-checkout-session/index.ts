...
Deno.serve(async (req) => {
  ...
  const siteUrl = getOptionalEnv('SITE_URL')?.replace(/\/+$/, '') ?? null
  const successUrl = getConfiguredCheckoutUrl('success')
    || body.successUrl
    || (siteUrl ? `${siteUrl}/checkout/success` : null)
  const cancelUrl = getConfiguredCheckoutUrl('cancel')
    || body.cancelUrl
    || (siteUrl ? `${siteUrl}/checkout/cancel` : null)
  if (!successUrl) return errorResponse(req, 'MISSING_SUCCESS_URL', 400)
  if (!cancelUrl) return errorResponse(req, 'MISSING_CANCEL_URL', 400)

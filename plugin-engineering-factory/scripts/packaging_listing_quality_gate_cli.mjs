...
import { runListingQualityGate } from "../src/packaging/premiumPackaging.mjs";

async function main() {
  const args = parseArgs(process.argv);
  if (!args.run) {
    throw new Error("Usage: node scripts/packaging_listing_quality_gate_cli.mjs --run runs/<run_id>");
  }
  ...
  try {
    lock = await acquireWorkflowRunLock({
      runDir,
      owner: {
        command: "packaging_listing_quality_gate_cli"
      },
  ...
  try {
    const result = await runListingQualityGate({
      projectRoot,
      runDir
    });
    console.log(JSON.stringify({
      ...
      premium_feel_score: result.report.premium_feel_score,

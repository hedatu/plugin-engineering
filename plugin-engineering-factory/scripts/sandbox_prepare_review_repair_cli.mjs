...
import { prepareReviewRepairPlan } from "../src/workflow/sandboxPostReview.mjs";

async function main() {
  const args = parseArgs(process.argv);
  if (!args.run || !args.reason) {
    throw new Error("Usage: node scripts/sandbox_prepare_review_repair_cli.mjs --run runs/<sandbox_validation_run_id> --reason \"<dashboard rejection reason>\"");
  }
  ...
  try {
    lock = await acquireWorkflowRunLock({
      runDir,
      owner: {
        command: "sandbox_prepare_review_repair_cli"
      },
  ...
  try {
    ...
    const report = await prepareReviewRepairPlan({
      runDir,
      rejectionReason: `${args.reason}`
    });
    console.log(`Review repair plan completed: ${runDir}`);

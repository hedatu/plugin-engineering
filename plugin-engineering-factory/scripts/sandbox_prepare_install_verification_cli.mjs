...
async function main() {
  ...
  if (!args.run) {
    throw new Error("Usage: node scripts/sandbox_prepare_install_verification_cli.mjs --run runs/<sandbox_validation_run_id>");
  }
  ...
  try {
    lock = await acquireWorkflowRunLock({
      runDir,
      owner: {
        command: "sandbox_prepare_install_verification_cli"
      },
  ...
  try {
    ...
    const report = await prepareInstallVerificationPlan({ runDir });
    console.log(`Install verification plan completed: ${runDir}`);

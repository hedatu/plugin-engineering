...
import { renderRemotionStills } from "../src/packaging/remotionFactory.mjs";

async function main() {
  const args = parseArgs(process.argv);
  if (!args.run) {
    throw new Error("Usage: node scripts/assets_remotion_stills_cli.mjs --run runs/<run_id>");
  }
  ...
  try {
    lock = await acquireWorkflowRunLock({
      runDir,
      owner: {
        command: "assets_remotion_stills_cli"
      },
  ...
  try {
    const result = await renderRemotionStills({
      projectRoot,
      runDir
    });
    console.log(JSON.stringify({
      ...
      dependency_status: result.report.dependency_status,
      generated_files: result.report.generated_files.length
    }, null, 2));
  } finally {

...
import { resolveResearchMore } from "../src/discovery/researchResolution.mjs";

async function main() {
  const args = parseArgs(process.argv);
  if (!args.run) {
    throw new Error("Usage: node scripts/discovery_resolve_research_more_cli.mjs --run runs/<daily_run_id>");
  }
  ...
  try {
    lock = await acquireWorkflowRunLock({
      runDir,
      owner: {
        command: "discovery_resolve_research_more_cli"
      },
  ...
  try {
    ...
    const result = await resolveResearchMore({ runDir });
    console.log(`Research resolution completed: ${runDir}`);
    console.log(`Final recommendation: ${redactSecretLikeText(result.resolution.final_recommendation)}`);
    console.log(`Selected wedge: ${redactSecretLikeText(result.resolution.selected_wedge_hypothesis.one_sentence_value)}`);
  } finally {

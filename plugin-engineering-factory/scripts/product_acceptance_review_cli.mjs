...
import { generateProductAcceptanceReview } from "../src/review/productQuality.mjs";

async function main() {
  const args = parseArgs(process.argv);
  if (!args.run) {
    throw new Error("Usage: node scripts/product_acceptance_review_cli.mjs --run runs/<sandbox_validation_run_id>");
  }
  ...
  try {
    lock = await acquireWorkflowRunLock({
      runDir,
      owner: {
        command: "product_acceptance_review_cli"
      },
  ...
  try {
    ...
    const result = await generateProductAcceptanceReview({ runDir });
    console.log(`Product acceptance review completed: ${runDir}`);
    console.log(`Acceptance status: ${result.report.acceptance_status}`);
    console.log(`Recommended decision: ${redactSecretLikeText(result.report.recommended_decision)}`);
  } finally {

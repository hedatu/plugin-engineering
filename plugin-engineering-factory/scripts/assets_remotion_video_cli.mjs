...
import { renderRemotionVideo } from "../src/packaging/remotionFactory.mjs";

async function main() {
  const args = parseArgs(process.argv);
  if (!args.run) {
    throw new Error("Usage: node scripts/assets_remotion_video_cli.mjs --run runs/<run_id>");
  }
  ...
  try {
    lock = await acquireWorkflowRunLock({
      runDir,
      owner: {
        command: "assets_remotion_video_cli"
      },
  ...
  try {
    ...
    console.log(JSON.stringify({
      ...
      video_status: result.report.video.status
    }, null, 2));
  } finally {

...
import { createProductRevisionPlan } from "../src/workflow/productRevision.mjs";

async function main() {
  const args = parseArgs(process.argv);
  if (!args.run || !args.reason) {
    throw new Error("Usage: node scripts/product_create_revision_plan_cli.mjs --run runs/<sandbox_validation_run_id> --reason \"<reason>\"");
  }
  ...
  try {
    lock = await acquireWorkflowRunLock({
      runDir,
      owner: {
        command: "product_create_revision_plan_cli"
      },
  ...
  try {
    ...
    const result = await createProductRevisionPlan({
      runDir,
      reason: `${args.reason}`
    });
    console.log(`Product revision plan completed: ${runDir}`);
    console.log(`Next step: ${redactSecretLikeText(result.report.next_step)}`);
  } finally {

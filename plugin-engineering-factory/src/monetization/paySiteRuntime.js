...
function membershipCopy(config, entitlementState, usageState, authState) {
  ...
  if (!authState.loggedIn) {
    return `Free plan includes ${config.freeLimit.amount} ${config.freeLimit.unit}. Log in with email to manage membership.`;
  }
...
export async function createPaySiteRuntime({
  ...
  async function requestUpgrade() {
    const authState = await refreshUi();
    if (!authState.loggedIn) {
      setText(statusNode, "Log in with email before upgrading.");
      return {
        ok: false,
        opened: false,
    ...
    if (!response?.ok) {
      if (response?.error === "LOGIN_REQUIRED") {
        setText(statusNode, "Log in with email before upgrading.");
      } else if (response?.error === "PLAN_NOT_FOUND" || response?.error === "PRODUCT_KEY_PENDING") {
        setText(statusNode, "Payment config is not ready yet.");
      } else {
        setText(statusNode, `Upgrade failed: ${response?.error ?? "CREATE_CHECKOUT_FAILED"}`);
      }
      return {
        ok: false,
        opened: false,
        error: response?.error ?? "CREATE_CHECKOUT_FAILED"
      };
    }
    await chrome.tabs.create({ url: response.data.checkoutUrl });
    setText(statusNode, "Checkout opened in a new tab. Pro unlocks only after webhook-confirmed entitlement.");
    return {
      ok: true,
      opened: true,

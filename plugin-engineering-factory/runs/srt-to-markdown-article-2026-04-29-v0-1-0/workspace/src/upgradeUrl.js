...
const INSTALLATION_KEY = "srt_md_installation_id";
const PRODUCT_KEY = "srt-to-markdown-article";
const BASE_PRICING_URL = "https://pay.915500.xyz/products/srt-to-markdown-article/pricing";

function randomId() {
  if (globalThis.crypto?.randomUUID) {
    return globalThis.crypto.randomUUID();
  }
  return `srt-${Date.now()}-${Math.random().toString(16).slice(2)}`;
}

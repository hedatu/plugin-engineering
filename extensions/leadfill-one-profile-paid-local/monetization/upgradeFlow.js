...
export async function createMonetizationRuntime({
  ...
  async function openUpgradePage() {
    const url = new URL(
      config.upgrade_url ?? "https://pay.915500.xyz/products/leadfill-one-profile/pricing"
    );
    url.searchParams.set("source", "chrome_extension");
    url.searchParams.set("productKey", config.product_key ?? config.product_id ?? "leadfill-one-profile");
    url.searchParams.set("planKey", config.plan_key ?? "lifetime");
